...
# 或者使用：WORK_DIR="/public/home/ghzhang/ablation_experiments"
...
submit_job() {
    ...
    # 创建输出目录（在提交前创建，避免权限问题）
    mkdir -p "$output_dir" 2>/dev/null || {
        echo -e "${RED}✗ 无法创建目录: $output_dir${NC}"
        echo -e "${YELLOW}  请检查路径权限或修改 WORK_DIR 变量${NC}"
        return 1
    }
...
# 清理模块环境（如果你的集群不使用module，可以注释掉这行）
# module purge
...
print_config_info() {
    ...
    if [ -n "$dependency_id" ]; then
        echo -e "${YELLOW}依赖作业:${NC}   $dependency_id (等待其完成)"
    else
...
if [ ! -d "$WORK_DIR" ]; then
    ...
    mkdir -p "$WORK_DIR" 2>/dev/null || {
        echo -e "${RED}✗ 无法创建工作目录: $WORK_DIR${NC}"
        echo -e "${YELLOW}  请修改脚本中的 WORK_DIR 变量为你有权限的目录${NC}"
        exit 1
    }
fi
...
for PROPERTY in "${PROPERTIES[@]}"; do
    for SEED in "${RANDOM_SEEDS[@]}"; do
        ...
        # ===== 配置1: 基线 =====
        OUTPUT_DIR="${WORK_DIR}/output_100epochs_${SEED}_bs64_sw_ju_${CONFIG_1_SUFFIX}_${PROPERTY}_quantext"
        ...
        OUTPUT_DIR="${WORK_DIR}/output_100epochs_${SEED}_bs64_sw_ju_${CONFIG_2_SUFFIX}_${PROPERTY}_quantext"
        ...
        # ===== 配置3: Fine-grained + Projection + Cross-modal, 无Middle =====
        OUTPUT_DIR="${WORK_DIR}/output_100epochs_${SEED}_bs64_sw_ju_${CONFIG_3_SUFFIX}_${PROPERTY}_quantext"
        ...
        # ===== 配置4: Full config =====
        OUTPUT_DIR="${WORK_DIR}/output_100epochs_${SEED}_bs64_sw_ju_${CONFIG_4_SUFFIX}_${PROPERTY}_quantext"
