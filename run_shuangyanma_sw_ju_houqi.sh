...
# 创建日志文件名（包含时间戳）
LOG_FILE="./output_100epochs_42_bs128_sw_ju_houqi/train_$(date +%Y%m%d_%H%M%S).log"

# 确保输出目录存在
mkdir -p ./output_100epochs_42_bs128_sw_ju_houqi
...
nohup python train_with_cross_modal_attention.py \
    ...
    --output_dir ./output_100epochs_42_bs128_sw_ju_houqi \
    --num_workers 24 \
    --random_seed 42 \
    > "$LOG_FILE" 2>&1 &
